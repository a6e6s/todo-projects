...
<body class="font-sans antialiased bg-slate-950" style="font-family: 'Cairo', ui-sans-serif, system-ui, sans-serif;">
    <div class="min-h-screen flex items-center justify-center px-6 relative overflow-hidden">
        ...
        <div class="absolute inset-0 overflow-hidden pointer-events-none">
            <div class="absolute top-20 ltr:left-10 rtl:right-10 w-32 h-32 bg-gradient-to-br from-orange-500/20 to-red-600/20 rounded-3xl blur-xl animate-pulse"></div>
            <div class="absolute bottom-20 ltr:right-10 rtl:left-10 w-40 h-40 bg-gradient-to-br from-purple-500/20 to-pink-600/20 rounded-3xl blur-2xl animate-pulse" style="animation-delay: 1s;"></div>
        </div>
        ...
        <div class="relative z-10 max-w-2xl w-full" x-data="{ show: false }" x-init="setTimeout(() => show = true, 100)">
            <div class="backdrop-blur-xl bg-white/5 border border-white/10 rounded-3xl p-12 text-center" 
                ...
                {{-- Logo --}}
                <div class="mb-8 opacity-50 transform -rotate-6">
                    <img src="{{ asset('logo.png') }}" alt="FluxFlow" class="w-40 mx-auto">
                </div>

                {{-- Error Code --}}
                <div class="text-8xl font-bold text-orange-500 mb-6 animate-pulse">500</div>

                {{-- Error Message --}}
                <h1 class="text-3xl font-bold text-white mb-4">Our Engine Hit a Snag</h1>
                <p class="text-xl text-white/70 mb-8">The Kanban board fell over. We're picking up the cards as fast as we can.</p>
