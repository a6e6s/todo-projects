            ...
            @if ($this->task)
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label
                                class="block text-sm font-medium text-slate-400 mb-2">{{ __('app.due_date') }}</label>
                            <div class="relative">
                                <x-lucide-calendar
                        ...
                        <div>
                            <label
                                class="block text-sm font-medium text-slate-400 mb-2">{{ __('app.effort_score') }}</label>
                            <div class="relative">
                                <x-lucide-gauge
                                    class="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-slate-500 pointer-events-none" />
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label
                                class="block text-sm font-medium text-slate-400 mb-2">{{ __('app.assignee') }}</label>
                        ...
                        <div>
                            <label
                                class="block text-sm font-medium text-slate-400 mb-2">{{ __('app.priority') }}</label>
                                ...
                                @foreach (['low' => ['label' => 'Low', 'color' => 'blue'], 'medium' => ['label' => 'Med', 'color' => 'amber'], 'high' => ['label' => 'High', 'color' => 'red']] as $value => $config)
                                    <button type="button" wire:click="setPriority('{{ $value }}')"
                                        @class([
                                            ...
                                            'bg-[#101a22] text-slate-500 hover:bg-[#283239]' => $priority->value !== $value,
                                        ])>
                                        {{ $config['label'] }}
                                    </button>
                                @endforeach
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <span
                                class="text-xs text-slate-500">{{ __('app.file_count', ['count' => $this->attachments->count()]) }}</span>
                        </div>
                        ...
                        @if (count($files) > 0)
                            <div class="mt-4 space-y-2">
                                ...
                                @foreach ($files as $index => $file)
                                    <div
                                        class="flex items-center justify-between p-3 bg-[#1c2630] rounded-lg border border-[#283239]">
                                        <div class="flex items-center gap-3">
                                ...
                                @foreach ($this->attachments as $attachment)
                                    <div wire:key="attachment-{{ $attachment->id }}"
                                        ...
                                        @else
                                            {{-- File Type Icon --}}
                                            <a href="{{ $attachment->url }}" target="_blank"
                                                class="block aspect-video flex items-center justify-center bg-[#101a22]">
                                                @php
